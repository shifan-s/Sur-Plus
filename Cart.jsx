...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    // Notifies other components (like a Navbar) that the cart has changed
  ...
  const changeQty = (cartId, delta) => {
    setCart(prev => {
      return prev
        ...
        .filter(item => item.qty > 0); // Logic: If qty becomes 0, filter it out
    });
  };
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                  ...
                  <div className="flex items-center gap-4 mt-4">
                    <button 
                      ...
                      className={`w-8 h-8 flex items-center justify-center font-bold rounded-full transition-colors ${
                        item.qty === 1 ? "bg-red-50 text-red-500 hover:bg-red-100" : "bg-gray-100 hover:bg-gray-200"
                      }`}
                    >
                      {item.qty === 1 ? "✕" : "−"}
                    </button>
                    <span className="w-4 font-bold text-center text-gray-700">{item.qty}</span>
                    <button 
                      onClick={() => changeQty(item.cartId, 1)} 
          ...
          <div className="lg:relative">
            <div className="sticky p-8 bg-white border shadow-2xl top-8 h-fit rounded-3xl border-blue-50">
              <h2 className="pb-4 mb-6 text-2xl font-bold text-gray-800 border-b">Order Summary</h2>
